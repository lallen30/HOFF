//
//  ForgotViewModel.swift
//  HOFFER
//
//  Created by JAM-E-214 on 25/04/24.
//  Copyright © 2024 Abdulkadir. All rights reserved.
//

import Foundation
final class ForgotViewModel {
    
    var forgotResponceDict = ForgotModel([String:Any]())
    var forgotErrorResponceDict = ForgotErrorModel([String:Any]())
    var eventHandler: ((_ event: Event) -> Void)? // Data Binding Closure

    func forgot(parameters: ForgotRequest) {
        self.eventHandler?(.loading)
        APIManager.shared.requestPost(
            modelType: ForgotModel.self, // response type
            type: APIEndPoint.forgotPassword(param: parameters),
            header: false) { result in
                self.eventHandler?(.stopLoading)
                switch result {
                case .success(let data):
                    print("Result >>>>>> \(result)")
                    print("Data >>>>>> \(data)")
                    self.forgotResponceDict = data
                    
                    self.eventHandler?(.dataLoaded)
                case .failure(let error):
                self.eventHandler?(.error(error))
                    print(error)
                }
                
            }
    }
    
}

