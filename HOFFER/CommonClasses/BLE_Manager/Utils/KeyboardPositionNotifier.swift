...
class KeyboardPositionNotifier: NSObject {
    ...
    private func registerNotifications(enabled: Bool) {
        ...
        if enabled {
//			keyboardWillBeShownObserver = notificationCenter.addObserver(forName: NSNotification.Name.UIResponder.keyboardWillShowNotification, object: nil, queue: .main, using: {[weak self] notification in self?.keyboardWillBeShown(notification: notification)})
//			keyboardWillBeHiddenObserver = notificationCenter.addObserver(forName: NSNotification.Name.keyboardWillHideNotification, object: nil, queue: .main, using: {[weak self] notification in self?.keyboardWillBeHidden(notification: notification)})
        } else {
