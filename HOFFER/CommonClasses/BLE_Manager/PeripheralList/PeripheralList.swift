...
class PeripheralList {
    ...
    func filtersDescription() -> String? {
        ...
        if let rssiFilterValue = rssiFilterValue {
            let rssiString = String(format: localizationManager.localizedString("RSSI >= %ld"), rssiFilterValue) //  "RSSI >= \(rssiFilterValue)"
        ...
        if !isUnnamedEnabled {
            let namedString = localizationManager.localizedString("with name")
            if filtersTitle != nil && !filtersTitle!.isEmpty {
        ...
        if isOnlyUartEnabled {
            let uartString = localizationManager.localizedString("with UART")
            if filtersTitle != nil && !filtersTitle!.isEmpty {
